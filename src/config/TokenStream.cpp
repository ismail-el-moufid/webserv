...
{
	if (done())
		throw std::runtime_error(tokens_.back().errorLocation(filePath) + "unexpected end of file");
...
{
	if (done())
		throw std::runtime_error(tokens_.back().errorLocation(filePath) + "unexpected end of file, expected " + typeName(type));
	if (tokens_.at(pos_).type != type)
		throw std::runtime_error(tokens_.at(pos_).errorLocation(filePath) + "unexpected token '" + tokens_.at(pos_).content + "', expected " + typeName(type));
...
{
	if (done())
		throw std::runtime_error(tokens_.back().errorLocation(filePath) + "unexpected end of file, expected '" + value + "'");
	if (tokens_.at(pos_).content != value)
		throw std::runtime_error(tokens_.at(pos_).errorLocation(filePath) + "unexpected token '" + tokens_.at(pos_).content + "', expected '" + value + "'");
	++pos_;
}
...
void Config::TokenStream::skipBlock()
{
	...
	throw std::runtime_error(tokens_.back().errorLocation(filePath) + "unexpected end of file, missing '}'");
}
...
std::string Config::TokenStream::Token::errorLocation(const std::string &filePath) const
{
	return filePath + ":" + StringUtils::toString(line) + ":" + StringUtils::toString(column) + ": error: ";
}
