...
{
	route.allowedMethods_.push_back("GET");
...
void Route::setRedirect(const std::string &code, const std::string &page)
{
	...
	if (!upload_.empty())
		throw std::runtime_error(StringUtils::currentTime() + " [error] redirect and upload are mutually exclusive and can't exist on the same location block");
	if (!HttpStatus::isRedirectCode(code))
		throw std::runtime_error(StringUtils::currentTime() + " [error] Invalid redirect code");
	redirect_		= true;
...
void Route::setUpload(const std::string &upload)
{
	if (redirect_)
		throw std::runtime_error(StringUtils::currentTime() + " [error] redirect and upload are mutually exclusive and can't exist on the same location block");
	if (!cgis_.empty())
		throw std::runtime_error(StringUtils::currentTime() + " [error] upload and cgi are mutually exclusive and can't exist on the same location block");
	upload_ = upload;
}
...
void Route::setAutoIndex(const std::string &autoindex)
{
	...
	else							throw std::runtime_error(StringUtils::currentTime() + " [error] autoIndex must be 'on' or 'off'");
}
...
void Route::addCgi(const std::string &extension, const std::string &path)
{
	if (redirect_)
		throw std::runtime_error(StringUtils::currentTime() + " [error] redirect and cgi are mutually exclusive and can't exist on the same location block");
	if (!upload_.empty())
		throw std::runtime_error(StringUtils::currentTime() + " [error] cgi and upload are mutually exclusive and can't exist on the same location block");
	cgis_[extension] = path;
}

void	Route::addIndexFile(const std::string &indexFile) { indexFiles_.push_back(indexFile); }
void	Route::clearAllowedMethods() { allowedMethods_.clear(); }
